...
public final class RaidGoldRewardBonus {
    private RaidGoldRewardBonus() {
    }

    public static int percentFromEffect(Effect effect) {
        return effect instanceof Effect.RaidGoldRewardPercent raid ? raid.percent() : 0;
    }
    ...
    /**
     * Same expiry rule as personage buffs: {@link GroupBuildingPassiveEffect#expiresAt()} empty = permanent;
     * otherwise counted only while {@code expiresAt.isAfter(now)}.
     */
