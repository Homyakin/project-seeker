...
@Component
public class AnomalyJoinExecutor extends CommandExecutor<AnomalyJoin> {
    private final GroupUserService groupUserService;
    private final AnomalyService anomalyService;
    private final TelegramAnomalyService telegramAnomalyService;
    private final TelegramSender telegramSender;

    public AnomalyJoinExecutor(
    ...
    @Override
    public void execute(AnomalyJoin command) {
        ...
        final var result = anomalyService.join(command.launchedEventId(), user.personageId());
        if (result.isLeft()) {
        ...
        telegramSender.send(TelegramMethods.createAnswerCallbackQuery(
            command.callbackId(),
            AnomalyLocalization.successJoin(group.language())
        ));
    }
}
