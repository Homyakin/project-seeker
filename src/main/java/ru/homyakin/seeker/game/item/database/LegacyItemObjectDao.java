...
@Component
public class LegacyItemObjectDao {
    private final JdbcClient jdbcClient;
    private final JsonUtils jsonUtils;
    private final ManyToManyUpdater updater;

    public LegacyItemObjectDao(DataSource dataSource, JsonUtils jsonUtils, ManyToManyUpdater updater) {
    ...
    @Transactional
    public void saveObject(LegacySavingItemObject object) {
        final var sql = """
            INSERT INTO legacy_item_object (code, characteristics, locale)
    ...
    public LegacyGenerateItemObject getRandomObject(LegacyItemRarity rarity, PersonageSlot slot) {
        final var sql = """
            WITH random_object AS (
               SELECT id FROM legacy_item_object io
                LEFT JOIN legacy_item_object_to_item_rarity iotir on io.id = iotir.item_object_id
            ...
            LEFT JOIN legacy_item_object io ON io.id = ro.id
            LEFT JOIN legacy_item_object_to_personage_slot iotps on io.id = iotps.item_object_id
            WHERE io.id = ro.id
            """;
    ...
    public LegacyGenerateItemObject getById(int id) {
        final var sql = """
            SELECT * FROM legacy_item_object io
            LEFT JOIN legacy_item_object_to_personage_slot iotps on io.id = iotps.item_object_id
             WHERE io.id = :id
            """;
        return jdbcClient.sql(sql)
    ...
    private void saveObjectSlots(int id, Set<PersonageSlot> slots) {
        updater.update(
            "legacy_item_object_to_personage_slot",
            "item_object_id",
            "personage_slot_id",
            id,
    ...
    private void saveObjectRarities(int id, Set<LegacyItemRarity> rarities) {
        updater.update(
            "legacy_item_object_to_item_rarity",
            "item_object_id",
            "item_rarity_id",
            id,
    ...
    private LegacyGenerateItemObject extractSingleObject(ResultSet rs) throws SQLException {
        rs.next();
        ...
        return new LegacyGenerateItemObject(
            id,
            code,
            slots,
            characteristics,
