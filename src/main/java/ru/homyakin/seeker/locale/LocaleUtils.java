...
public class LocaleUtils {
    public static String groupNameWithBadge(Group group) {
        return groupNameWithBadge(group.badge(), group.tag(), group.name());
    }

    public static String groupNameWithBadge(GroupTopPosition position) {
        return groupNameWithBadge(position.badge(), position.tag(), position.name());
    }

    public static String groupTagWithBadge(Group group) {
        return group.badge().icon() + tagString(group.tag());
    }

    public static String groupNameWithBadge(GroupProfile profile) {
        return groupNameWithBadge(profile.badge(), profile.tag(), profile.name());
    }

    private static String groupNameWithBadge(
        BadgeView badge,
        Optional<String> tag,
        String name
    ) {
