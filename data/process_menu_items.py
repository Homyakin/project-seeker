...
def process_menu_item(menu_item: Dict):
    ...
    database.put(menu_item, table='menu_item', pk_columns=['id'], simple_columns=['price', 'is_available', 'category_id'])

    if 'locale' in menu_item:
        process_locale(menu_item['locale'], menu_item["id"])
    else:
        raise Exception(f'Menu item with id {menu_item["id"]} must contain locale')
